...
dfc-core is at the bottom of the DfcBlockChain stack, many other services can be
 built on top of it.
...
# Integration points
## Data consumers
Many types of services can be built based on the data generated by dfc-core.

dfc-core generates two types of data that can be queried by applications.

Unless otherwise noted, data stored is `validated`, ie data is only persisted
after consensus is reached on the network.

An overview of how the data flows within dfc-core is [this data flow presentation](software/core-data-flow.pdf)
...
properly registered by updating its cursor so that dfc-core can safely
garbage collect data no longer needed.
By default dfc-core only keeps track of its internal state and will
 ...
 * additional meta data not available in history is produced by dfc-core.
 ...
 * does not depend on running dfc-core (can be built as a pure client
...
### DfcBlockChain core database

## API to dfc-core
  dfc-core exposes a simple HTTP based API for performing various
  types of operations on dfc-core itself or on entities in the network
