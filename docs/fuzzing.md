...
fuzzer with dfc-core. Support for this is still preliminary but it has
...
small inputs in binary form. We've modified dfc-core to have a mode for (b)
...
DfcBlockChainMessage into them. This is quite a lot of setup and the fuzzing speed of
...
## Building an instrumented dfc-core
...
The simplest way is `make fuzz`; this will do the following:
  ...
  - Run `dfc-core --genfuzz fuzz-testcases/fuzz$i.xdr` ten times to produce
    some basic seed input for the corpus.
  - Create a directory `fuzz-findings` for storing crash-producing inputs.
  - Run `afl-fuzz` on `dfc-core --fuzz`, using those corpus directories.
...
by feeding it to an instance of `dfc-core --fuzz` run by hand, elsewhere (in
particular, not fighting for control over tmpdirs with the fuzzer's
`dfc-core` instances). Often a fuzzer "crash" is just the subprocess hitting
...
`dfc-core` in a different directory with `--enable-asan`, or valgrind, in
...
part of staging-tests", here are some directions I think we should take fuzzing:
  ...
  - Try limiting the instrumentation to `dfc-core` itself, not libsodium,
  ...
  - Make startup-modes at different points in the process: instantiate an
    ...
    `DfcBlockChainMessage`s. Let the fuzzer generte bucket ledger entries, and try to
  ...
  - Add a mode -- with a giant red flashing TESTING_ONLY light on it -- that
    ...
    as an attacker who figures out how to break `dfc-core` will be able
