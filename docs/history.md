...
DfcBlockChain Core, or dfc-core, separates data into "current" and "historical."

Current data is the subject of peer-to-peer messages--consensus is only concerned with the present,
not the past. Current data _resides_ in a local SQL database paired with each dfc-core
process. This database is consulted and updated "live" in an ACID fashion as dfc-core applies
each [transaction](transaction.md) set for which consensus was reached and forms each new [ledger](ledger.md).

Unlike many similar systems, dfc-core does _not_ need to consult history in order to apply a
...
Historical data resides in _history archives_. A dfc-core process should almost always have
access to a history archive. While it is possible to run a dfc-core process without any
...
normal operations, a dfc-core process should always be configured with one or more history
archives.
...
Many different facilities or services can be used as history archives; dfc-core only needs to be
supplied with a way to "get" and "put" files to and from the archive. For example, history archives
may be FTP or SFTP servers, filesystem directories shared between dfc-core processes, AWS S3,
Google Cloud Storage, Azure Blob storage or similar commodity object storage services.

History archives are defined in a very lightweight fashion, in dfc-core's configuration file, by
providing a pair of `get` and `put` command templates. dfc-core will run the provided command
...
dfc-core leans heavily on the XDR data format. This is an old, stable, standardized
...
When storing XDR files to history archives, dfc-core first applies gzip (RFC 1952) compression
to the files. The resulting `.xdr.gz` files can be concatenated, accessed in streaming fashion, or
decompressed to `.xdr` files and dumped as plain text by dfc-core.


## Checkpointing

During normal operation, a validating dfc-core server will save a "checkpoint" of its recent
...
When dfc-core finds that it is out of sync with its peers--either because it is joining
...
another when out of sync. If you run a dfc-core server without configuring history archives, it
will never synchronize with its peers (unless they all start at the same time).

The peer-to-peer protocol among dfc-core peers deals only with the current ledger's transactions
...
One of the factors motivating the dfc-core history design was to permit other programs and 3rd
parties transparent, easy, and unbiased access to the ledger and transaction history, without having
to "go through" the dfc-core program or protocol. Any program that can fetch data from a history
archive and deserialize XDR can read the complete history; there is no need to speak the
dfc-core peer-to-peer protocol or interact with any dfc-core peers.
...
sequentially. Anyone wishing to audit or reconstruct the activity of dfc-core by monitoring a
history archive can simply poll the archive and consume new files as they arrive.

All XDR encoding and decoding in dfc-core is done by code generated automatically from the
associated [XDR schemas](/src/xdr); any other compliant XDR code generator should produce a
deserializer that can read and write the same history. The XDR code generator used in dfc-core
...
few additional, less obvious motives are at work in the design of the history system in dfc-core.
A few reasons that the extra effort of configuring independent history archives is, in our judgment, worth its slight awkwardness:
  ...
  - This design reduces the risk of lost peers. dfc-core peers are comparatively ephemeral: new ones can
    ...
    been archived). While dfc-core is designed to run as a highly fault-tolerant replicated
...
`.well-known/dfc-history.json`. This is kept in the RFC 5785 `.well-known` directory, and is
...
In total, each checkpoint number `0xwwxxyyzz` consists of the following files:
  ...
  - One ledger-headers file, named by ledger number as `ledger/ww/xx/yy/ledger-wwxxyyzz.xdr.gz`. The
    ...
    [`LedgerHeaderHistoryEntry`](/src/xdr/DfcBlockChain-ledger.x), one per ledger in the checkpoint (so
  ...
  - One transactions file, named by ledger number as
    ...
    structures of the type [`TransactionHistoryEntry`](/src/xdr/DfcBlockChain-ledger.x), with zero-or-more
  ...
  - One results file, named by ledger number as `results/ww/xx/yy/results-wwxxyyzz.xdr.gz`. The file
    ...
    [`TransactionHistoryResultEntry`](/src/xdr/DfcBlockChain-ledger.x), with zero-or-more structures per
    ...
    actually running the `dfc-core` transaction-apply logic.

  - (Optionally) one SCP file, named by ledger number as `scp/ww/xx/yy/scp-wwxxyyzz.xdr.gz`. The file
    contains a sequence of XDR structures of the type
    [`SCPHistoryEntry`](/src/xdr/DfcBlockChain-ledger.x), with zero-or-more structures per ledger. The
