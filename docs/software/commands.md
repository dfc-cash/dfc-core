...
`$ dfc-core convert-id SDQVDISRYN2JXBS7ICL7QJAEKB3HWBJFP2QECXG7GZICAHBK4UNJCWK2`
...
* **force-scp**: This command is used to start a network from scratch or when a
  ...
  in the database. The next time dfc-core is run, dfc-core will start
  emitting SCP messages based on its last known ledger. Without this flag
  dfc-core waits to hear a ledger close from the network before starting
...
* **http-command <COMMAND>** Send an [HTTP command](#http-commands) to an
  already running local instance of dfc-core and then exit. For example: 

`$ dfc-core http-command info`
...
* **new-hist <HISTORY-LABEL> ...**:  Initialize the named history archives
  ...
  specified in the dfc-core.cfg. This will write a
  `.well-known/dfc-history.json` file in the archive root.
...
* **run**: Runs dfc-core service.
* **sec-to-pub**:  Reads a secret key on standard input and outputs the
  corresponding public key.  Both keys are in DfcBlockChain's standard
...
* **sign-transaction <FILE-NAME>**:  Add a digital signature to a transaction
  ...
  if the signing key appears to be coming from a terminal, dfc-core
  disables echo. Note that if you do not have a DFCBlockChain_NETWORK_ID environment
  variable, then before this argument you must specify the --netid option. For
  example, the production dfc network is "`Public Global DfcBlockChain Network ;
  September 2015`" while the test network is "`Test SDF Network ; September
  2015`".<br>
  Option --base64 alters the behavior to work on base64-encoded XDR rather than
  raw XDR.
* **test**: Run all the unit tests.
  * Suboptions specific to dfc-core:
  ...
  * For example this will run just the tests tagged with `[tx]` using protocol
    ...
    `dfc-core --test -a --version 9 --version 10 "[tx]"`
* **version**: Print version info and then exit.
* **write-quorum**: Print a quorum set graph from history.

## HTTP Commands
By default dfc-core listens for connections from localhost on port 11626. 
You can send commands to dfc-core via a web browser, curl, or using the --c 
...
* **setcursor**
  ...
  to tell dfc-core which data can be safely deleted by the instance. The
