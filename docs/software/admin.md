...
This document describes various aspects of running `dfc-core` for **system administrators** (but may be useful to a broader audience).

## Introduction

DfcBlockChain Core is responsible for communicating directly with and maintaining the DfcBlockChain peer-to-peer network. For a high-level introduction to DfcBlockChain Core, [watch this talk](https://www.youtube.com/watch?v=pt_mm8S9_WU) on the architecture and ledger basics:

[![Introduction to DfcBlockChain Core](https://i.ytimg.com/vi/pt_mm8S9_WU/hqdefault.jpg "Introduction to DfcBlockChain Core")](https://www.youtube.com/watch?v=pt_mm8S9_WU)

It will also be useful to understand how [data flows](https://www.dfc.org/developers/dfc-core/software/core-data-flow.pdf) and is stored in the system.

## Zero to completed: node checklist

  - [ ] [deciding to run a node](#why-run-a-node)
  - [ ] [setting up an instance to run core](#instance-setup)
  - [ ] [install dfc-core](#installing)
...
note: in this document we use "Horizon" as the example implementation of a first tier service built on top of dfc-core, but any other system would get the same benefits.
...
## Instance setup
Regardless of how you install dfc-core (apt, source, docker, etc), you will need to configure the instance hosting it roughly the same way.
...
As of early 2018, dfc-core with PostgreSQL running on the same machine has no problem running on a [m5.large](https://aws.amazon.com/ec2/instance-types/m5/) in AWS (dual core 2.5 GHz Intel Xeon, 8 GB RAM).
...
#### Interaction with the peer to peer network
  ...
  * **inbound**: dfc-core needs to allow all ips to connect to its `PEER_PORT` (default 11625) over TCP.
  * **outbound**: dfc-core needs access to connect to other peers on the internet on `PEER_PORT` (most use the default as well) over TCP.

#### Interaction with other internal systems

  * **outbound**:
    * dfc-core needs access to a database (postgresql for example), which may reside on a different machine on the network
    * other connections can safely be blocked
  * **inbound**: dfc-core exposes an *unauthenticated* HTTP endpoint on port `HTTP_PORT` (default 11626)
...
In general you should aim to run the latest [release](https://github.com/dfc/dfc-core/releases) as builds are backward compatible and are cummulative.
...
See the [INSTALL](https://github.com/dfc/dfc-core/blob/master/INSTALL.md) for build instructions.

### Package based Installation
If you are using Ubuntu 16.04 LTS we provide the latest stable releases of [dfc-core](https://github.com/dfc/dfc-core) and [dfc-horizon](https://github.com/dfc/go/tree/master/services/horizon) in Debian binary package format.

See [detailed installation instructions](https://github.com/dfc/packages#sdf---packages)

### Container based installation
Docker images are maintained in a few places, good starting points are:

   * the [quickstart image](https://github.com/dfc/docker-dfc-core-horizon)
   * the [standalone image](https://github.com/dfc/docker-dfc-core). **Warning**: this only tracks the latest master, so you have to find the image based on the [release](https://github.com/dfc/dfc-core/releases) that you want to use.

## Configuring

Before attempting to configure dfc-core, it is highly recommended to first try running a private network or joining the test network. 

### Configuration basics
All configuration for dfc-core is done with a TOML file. By default 
dfc-core loads `./dfc-core.cfg`, but you can specify a different file to load on the command line:

`$ dfc-core --conf betterfile.cfg <COMMAND>`

The [example config](https://github.com/dfc/dfc-core/blob/master/docs/dfc-core_example.cfg) is not a real configuration, but documents all possible configuration elements as well as their default values.

Here is an [example test network config](https://github.com/dfc/docker-dfc-core-horizon/blob/master/testnet/core/etc/dfc-core.cfg) for connecting to the test network.

Here is an [example public network config](https://github.com/dfc/docs/blob/master/other/dfc-core-validator-example.cfg) for connecting to the public network.
...
This section describes how to configure the quorum set for a validator and assumes basic understanding of the [DfcBlockChain Consensus Protocol](https://www.dfc.org/developers/guides/concepts/scp.html).

#### Validator list

You will find lists of validators in a few places:

  * [list of validators](https://github.com/dfc/docs/blob/master/validators.md)
  * the [DfcBlockChain Dashboard](https://dashboard.dfc.org/)

#### Understanding requirements for a good quorum

The way quorum sets are configured is explained in detail in the [example config](https://github.com/dfc/dfc-core/blob/master/docs/dfc-core_example.cfg).
...
Cross reference your validator settings, in particular:
  ...
  * [Automatic maintenance](#cursors-and-automatic-maintenance) configured properly, especially when dfc-core is used in conjunction with a downstream system like Horizon.

### Database and local state

After configuring your [database](#database) and [buckets](#buckets) settings, when running dfc-core for the first time, you must initialize the database:
...
#### Database
DfcBlockChain-core stores the state of the ledger in a SQL database.
...
see the [example config](https://github.com/dfc/dfc-core/blob/master/docs/dfc-core_example.cfg).
...
By default, dfc-core will perform this automatic maintenance, so be sure to disable it until you have done the appropriate data ingestion in downstream systems (Horizon for example sometimes needs to reingest data).
...
#### Buckets
DfcBlockChain-core stores a duplicate copy of the ledger in the form of flat XDR files 
...
Buckets should be stored on a fast local disk with sufficient space to store several times the size of the current ledger. 
 ...
 For the most part, the contents of both directories can be ignored as they are managed by dfc-core.

### History archives
DfcBlockChain-core normally interacts with one or more "history archives," which are 
...
The [example config](https://github.com/dfc/dfc-core/blob/master/docs/dfc-core_example.cfg) 
shows how to configure a history archive through command templates. 

While it is possible to run a dfc-core node with no configured history 
...
`$ dfc-core new-hist <historyarchive>`
...
After having configured your node and its environment, you're ready to start dfc-core.
...
Review the [logging](#logging) section to get yourself familiar with the output of dfc-core.

### Interacting with your instance
While running, interaction with dfc-core is done via an administrative 
HTTP endpoint. Commands can be submitted using command-line HTTP tools such 
as `curl`, or by running a command such as

`$ dfc-core http-command <http-command>`

The endpoint is [not intended to be exposed to the public internet](#interaction-with-other-internal-systems). It's typically accessed by administrators, or by a mid-tier application to submit transactions to the DfcBlockChain network. 
...
## Logging
DfcBlockChain-core sends logs to standard output and `dfc-core.log` by default, 
...
`$ dfc-core http-command "ll?level=debug"`
...
`$ dfc-core http-command "ll?level=debug&partition=history"` 
...
Run `$ dfc-core http-command 'info'`
The output will look something like
```json
{
      "build" : "v10.2.0",
      ...
      "network" : "Public Global DfcBlockChain Network ; September 2015",
      "peers" : {
...
Additional fields typically used by downstream systems:
  ...
  * `build` is the build number for this dfc-core instance
...
`$ dfc-core http-command 'quorum?node=$sdf1` or `$ dfc-core http-command 'quorum?node=@GABCDE` 
...
Example here is to upgrade the protocol version to version 9 on January-31-2018.
  ...
  1. `$ dfc-core http-command 'upgrades?mode=set&upgradetime=2018-01-31T20:00:00Z&protocolversion=9'`
  2. `$ dfc-core http-command info`
...
DfcBlockChain-core can be started directly from the command line, or through a supervision 
system such as `init`, `upstart`, or `systemd`.

DfcBlockChain-core can be gracefully exited at any time by delivering `SIGINT` or
...
DfcBlockChain-core can also be packaged in a container system such as Docker, so long 
as `BUCKET_DIR_PATH` and the database are stored on persistent volumes. For an
example, see [docker-dfc-core](https://github.com/dfc/docker-dfc-core-horizon).

### In depth architecture

[architecture.md](https://github.com/dfc/dfc-core/blob/master/docs/architecture.md) 
  describes how dfc-core is structured internally, how it is intended to be 
