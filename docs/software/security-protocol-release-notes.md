...
This document describes changes to the DfcBlockChain protocol as well as other changes made to address security issues.
...
* Changes to SCP.
* Changes to the DfcBlockChain Protocol (anything that modifies how the distributed ledger functions, including historical data).

## Security issues

* DDoS.
* Crashes (that could lead to remote code execution).
* Other attacks that can be exploited (inside or outside of the DfcBlockChain protocol).

## Goals for this document

* have a summary view of changes that affect the code base (dfc-core has to be able to replay all ledgers generated since genesis on the DfcBlockChain public network).
...
* this document is not where security disclosures are made, instead follow the process outlined in [DfcBlockChain's bug bounty program](https://www.dfc.org/bug-bounty-program/) as a way to triage and respond to issues.
* issues are reflected in this document 30 days after release of the version of DfcBlockChain core containing fixes for the issues.
