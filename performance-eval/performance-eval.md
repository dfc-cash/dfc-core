...
* Disk I/O at the operating system level (ie: unit is typically in blocks/s), both reads and writes of the dfc-core process
...
dfc-core has a built-in load generator that allows to inject transactions on private networks.
...
See [docs/metrics](docs/metrics.md) for information on metrics exposed by dfc-core.
...
* Use your own version of dependencies (sqlite, etc) with the same compile options (done for you by dfc-core's configure script) 
...
# alternatively, you can start "dfc-core", with something like
LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libtcmalloc_minimal.so ./dfc-core ...
```
...
This example grabs 60 seconds of data from a running dfc-core instance.

    perf record -F 99 -p $(pgrep dfc-core) --call-graph dwarf,20000 -o perf-cpu.data -- sleep 60
...
Records for 60 seconds events related to when a running dfc-core process is waiting on something.
...
Note: "counts" in this report represent time in milliseconds.
    ...
    perf record -e 'sched:sched_stat_sleep,sched:sched_switch,sched:sched_process_exit' -p $(pgrep dfc-core) --call-graph dwarf -o perf-offcpu-raw.data  -- sleep 60
