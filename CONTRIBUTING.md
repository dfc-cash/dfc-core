...
dfc.org also uses these same github issues to keep track of what we are working on. If you see any issues that are assigned to a particular person that means someone is currently working on that issue. 

Of course feel free to make your own issues if you think something needs to added or fixed.

# Basic quality checks

Please ensure that all tests pass before submitting changes. The local testsuite can be run as `make check` or `src/dfc-core --test`,
...
All content, comments, and pull requests must follow the [DfcBlockChain Community Guidelines](https://www.dfc.org/community-guidelines/). 
...
run one test with:
  `src/dfc-core --test  testName`

run one test category with:
  `src/dfc-core --test '[categoryName]'`
...
supported test options can be seen with
  `src/dfc-core --test --help`

display tests timing information:
  `src/dfc-core --test -d yes '[categoryName]'`

xml test output (includes nested section information):
  `src/dfc-core --test -r xml '[categoryName]'`
...
Then, running
* `dfc-core --test [stress]` will run all the stress tests,
* `dfc-core --test [foo-stress]` will run the stress tests for subsystem foo alone, and
* neither `dfc-core --test` nor `dfc-core --test [foo]` will run stress tests.
