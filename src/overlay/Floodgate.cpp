...
namespace dfc
{
Floodgate::FloodRecord::FloodRecord(DfcBlockChainMessage const& msg, uint32_t ledger,
                                    Peer::pointer peer)
...
bool
Floodgate::addRecord(DfcBlockChainMessage const& msg, Peer::pointer peer)
{
    if (mShuttingDown)
    {
        return false;
...
void
Floodgate::broadcast(DfcBlockChainMessage const& msg, bool force)
{
    if (mShuttingDown)
    {
        return;
