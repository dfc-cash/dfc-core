...
static std::string
msgSummary(DfcBlockChainMessage const& msg)
{
    switch (msg.type())
    {
    case ERROR_MSG:
...
void
Peer::sendMessage(DfcBlockChainMessage const& msg)
{
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            << "("
...
void
Peer::recvMessage(DfcBlockChainMessage const& dfcMsg)
{
    if (shouldAbort())
    {
        return;
    ...
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            ...
            << mApp.getConfig().toShortString(
                   mApp.getConfig().NODE_SEED.getPublicKey())
            << ") recv: " << msgSummary(dfcMsg)
            << " from:" << mApp.getConfig().toShortString(mPeerID);

    if (!isAuthenticated() && (dfcMsg.type() != HELLO) &&
        (dfcMsg.type() != AUTH) && (dfcMsg.type() != ERROR_MSG))
    {
        CLOG(WARNING, "Overlay")
            << "recv: " << dfcMsg.type() << " before completed handshake";
        drop(Peer::DropMode::IGNORE_WRITE_QUEUE);
        return;
    }

    assert(isAuthenticated() || dfcMsg.type() == HELLO ||
           dfcMsg.type() == AUTH || dfcMsg.type() == ERROR_MSG);
...
void
Peer::recvDontHave(DfcBlockChainMessage const& msg)
{
...
void
Peer::recvGetTxSet(DfcBlockChainMessage const& msg)
{
    auto self = shared_from_this();
...
void
Peer::recvTxSet(DfcBlockChainMessage const& msg)
{
...
void
Peer::recvTransaction(DfcBlockChainMessage const& msg)
{
...
void
Peer::recvGetSCPQuorumSet(DfcBlockChainMessage const& msg)
{
...
void
Peer::recvSCPQuorumSet(DfcBlockChainMessage const& msg)
{
...
void
Peer::recvSCPMessage(DfcBlockChainMessage const& msg)
{
...
void
Peer::recvGetSCPState(DfcBlockChainMessage const& msg)
{
...
void
Peer::recvError(DfcBlockChainMessage const& msg)
{
...
void
Peer::recvAuth(DfcBlockChainMessage const& msg)
{
    if (mState != GOT_HELLO)
    {
...
void
Peer::recvGetPeers(DfcBlockChainMessage const& msg)
{
    sendPeers();
}

void
Peer::recvPeers(DfcBlockChainMessage const& msg)
{
