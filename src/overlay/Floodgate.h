...
class Floodgate
{
    class FloodRecord
    {
      public:
        ...
        uint32_t mLedgerSeq;
        DfcBlockChainMessage mMessage;
        std::set<std::string> mPeersTold;

        FloodRecord(DfcBlockChainMessage const& msg, uint32_t ledger,
                    Peer::pointer peer);
    };
  ...
  public:
    ...
    bool addRecord(DfcBlockChainMessage const& msg, Peer::pointer fromPeer);

    void broadcast(DfcBlockChainMessage const& msg, bool force);
