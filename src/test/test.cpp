...
bool force_sqlite = (std::getenv("DFCBlockChain_FORCE_SQLITE") != nullptr);
...
getTestConfig(int instanceNumber, Config::TestDbMode mode)
{
    ...
    {
        gTestRoots.emplace_back("dfc-core-test");
        ...
        sstream << "dfc" << instanceNumber << ".log";
     ...
     std::vector<std::string> const& metrics)
{
    ...
    // not mixed with dfc-core logging.
    Logging::setFmt("<test>");
    ...
    LOG(INFO) << "Testing dfc-core " << DFCBlockChain_CORE_VERSION;
    ...
    cli |= clara::Opt(gBaseInstance, "offset")["--base-instance"](
        ...
        "dfc-core can run tests concurrently");
    session.cli(cli);
