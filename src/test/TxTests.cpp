              ...
              std::vector<TransactionFramePtr> const& txs)
{
    ...
    DfcBlockChainValue sv(txSet->getContentsHash(), getTestDate(day, month, year),
                    emptyUpgradeSteps, 0);
...
{
    auto res = dfc::loadAccountWithoutRecord(ltx, k);
    if (mustExist)
    {
        REQUIRE(res);
    }
...
{
    LedgerTxn ltx(app.getLedgerTxnRoot());
    return (bool)dfc::loadAccountWithoutRecord(ltx, k);
}
...
{
    LedgerTxn ltx(app.getLedgerTxnRoot());
    auto account = dfc::loadAccount(ltx, k);
                       ...
                       SequenceNumber seq)
{
    ...
    case MANAGE_OFFER_UPDATED:
    {
        LedgerTxn ltx(app.getLedgerTxnRoot());
        auto offer =
            dfc::loadOffer(ltx, source.getPublicKey(), expectedOfferID);
        REQUIRE(offer);
                        ...
                        ManageOfferEffect expectedEffect)
{
    ...
    if (createPassiveOfferResult.code() == MANAGE_OFFER_SUCCESS)
    {
        ...
        {
            LedgerTxn ltx(app.getLedgerTxnRoot());
            REQUIRE(!dfc::loadOffer(ltx, source.getPublicKey(),
                                        expectedOfferID));
        }
        break;
        default:
