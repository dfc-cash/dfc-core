...
TestAccount::updateSequenceNumber()
{
    if (mSn == 0)
    {
        ...
        auto entry = dfc::loadAccount(ltx, getPublicKey());
        if (entry)
        {
...
TestAccount::create(SecretKey const& secretKey, uint64_t initialBalance)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto entry = dfc::loadAccount(ltx, publicKey);
        if (entry)
        {
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto destAfter = dfc::loadAccount(ltx, publicKey);
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        REQUIRE(dfc::loadAccount(ltx, publicKey));
    }
...
TestAccount::merge(PublicKey const& into)
{
    ...
    LedgerTxn ltx(mApp.getLedgerTxnRoot());
    REQUIRE(dfc::loadAccount(ltx, into));
    REQUIRE(!dfc::loadAccount(ltx, getPublicKey()));
}
...
TestAccount::manageData(std::string const& name, DataValue* value)
{
    ...
    auto data = dfc::loadData(ls, getPublicKey(), name);
    if (value)
    {
        REQUIRE(data);
...
TestAccount::pay(PublicKey const& destination, int64_t amount)
{
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountEntry = dfc::loadAccount(ltx, destination);
        toAccount =
            toAccountEntry
    ...
    {
        LedgerTxn ltx(mApp.getLedgerTxnRoot());
        auto toAccountAfter = dfc::loadAccount(ltx, destination);
        // check that the target account didn't change
