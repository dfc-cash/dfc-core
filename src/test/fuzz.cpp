// Copyright 2015 DfcBlockChain Development Foundation and contributors. Licensed
...
#include "main/DfcBlockChainCoreVersion.h"
 ...
 *     random DfcBlockChainMessages. This is the mode you use to generate seed data
 *     for the external fuzzer's corpus.
 *
 *   - In --fuzz mode it reads back a file and appplies it to a pair of of
 *     dfc-cores in loopback mode, cranking the I/O loop to simulate
...
std::string
msgSummary(DfcBlockChainMessage const& m)
{
...
bool
tryRead(XDRInputFileStream& in, DfcBlockChainMessage& m)
{
    try
    {
        return in.readOne(m);
     ...
     std::vector<std::string> const& metrics)
{
    ...
    LOG(INFO) << "Fuzzing dfc-core " << DFCBlockChain_CORE_VERSION;
...
genfuzz(std::string const& filename)
{
    ...
    out.open(filename);
    autocheck::generator<DfcBlockChainMessage> gen;
    for (size_t i = 0; i < n; ++i)
    {
        try
        {
            DfcBlockChainMessage m(gen(10));
            out.writeOne(m);
