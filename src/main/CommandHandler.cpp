...
{
    retStr = "<b>Welcome to dfc-core!</b><p>";
    ...
    retStr +=
        ...
        "Cursors are used by dependent services to tell dfc - core which "
...
CommandHandler::manualClose(std::string const& params, std::string& retStr)
{
    ...
    {
        retStr =
            "Set MANUAL_CLOSE=true in the dfc-core.cfg if you want this "
            "behavior";
    }
}
...
CommandHandler::generateLoad(std::string const& params, std::string& retStr)
{
    ...
    else
    {
        retStr = "Set ARTIFICIALLY_GENERATE_LOAD_FOR_TESTING=true in "
                 "the dfc-core.cfg if you want this behavior";
    }
}
...
CommandHandler::testAcc(std::string const& params, std::string& retStr)
{
    ...
    else
    {
        ...
        auto acc = dfc::loadAccount(ltx, key.getPublicKey());
        if (acc)
        {
