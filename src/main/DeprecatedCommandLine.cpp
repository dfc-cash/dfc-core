...
const struct option dfc_core_options[] = {
...
usage(int err = 1)
{
    ...
    os << "usage: dfc-core [OPTIONS]\n"
          ...
          "      --forcescp           Next time dfc-core is run, SCP will "
          "start "
          ...
          "signtxn\n"
          "                           (Default is DFCBlockChain_NETWORK_ID "
...
handleDeprecatedCommandLine(int argc, char* const* argv)
{
    ...
    std::string cfgFile("dfc-core.cfg");
    std::string command;
    ...
    int opt;
    while ((opt = getopt_long_only(argc, argv, "c:", dfc_core_options,
                                   nullptr)) != -1)
    {
        switch (opt)
        {
...
#endif
        case OPT_VERSION:
            std::cout << DFCBlockChain_CORE_VERSION << std::endl;
            return 0;
        case OPT_HELP:
        default:
            usage(0);
