# What is a ledger?
A ledger represents the state of the DfcBlockChain universe at a given point in time.
...
[`src/xdr/DfcBlockChain-ledger.x`](../xdr/DfcBlockChain-ledger.x)
...
During consensus, nodes work together to decide on the value of DfcBlockChainValue.
DfcBlockChainValue is then saved in the scpValue field of the ledger header.
Any node on the network, given the previous ledger (their previous state) and
'DfcBlockChainValue' should be able to transition to the same new ledger.
...
[`src/xdr/DfcBlockChain-ledger-entries.x`](../xdr/DfcBlockChain-ledger-entries.x)

### AccountEntry
This entry represents an account. In DfcBlockChain, everything is centered around
...
Offers are a way to automate simple trading inside the DfcBlockChain network.
