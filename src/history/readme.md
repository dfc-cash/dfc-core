...
The `get` and `put` commands define subprocesses that `dfc-core` will
...
called `s3dfc` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3dfc]
get="curl http://history.dfc.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.dfc.org/{1}"
~~~~

In this example, `dfc-core` will use the `curl` command to fetch history and
