...
{
    auto values = getDfcBlockChainValues(envelope.statement);
    auto result = std::vector<Hash>{};
    result.resize(values.size());

    std::transform(std::begin(values), std::end(values), std::begin(result),
                   [](DfcBlockChainValue const& sv) { return sv.txSetHash; });

    return result;
}

std::vector<DfcBlockChainValue>
getDfcBlockChainValues(SCPStatement const& statement)
{
    auto values = Slot::getStatementValues(statement);
    auto result = std::vector<DfcBlockChainValue>{};
    result.resize(values.size());

    std::transform(std::begin(values), std::end(values), std::begin(result),
                   [](Value const& v) {
                       auto wb = DfcBlockChainValue{};
