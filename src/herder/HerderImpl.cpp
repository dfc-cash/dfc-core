...
void
HerderImpl::valueExternalized(uint64 slotIndex, DfcBlockChainValue const& value)
{
    // record metrics
...
HerderImpl::recvTransaction(TransactionFramePtr tx)
{
...

    {
        ...
        auto sourceAccount = dfc::loadAccount(ltx, tx->getSourceID());
...
HerderImpl::triggerNextLedger(uint32_t ledgerSeqToTrigger)
{
    ...
    DfcBlockChainValue newProposedValue(txSetHash, nextCloseTime, emptyUpgradeSteps,
                                  0);
    ...
    std::vector<TransactionFramePtr> const& applied)
{
    ...
    // propagation
    {
        ...
        {
            auto msg = tx->toDfcBlockChainMessage();
