...
void
save(Archive& ar, dfc::Upgrades::UpgradeParameters const& p)
{
    ar(make_nvp("time", dfc::VirtualClock::to_time_t(p.mUpgradeTime)));
...
template <class Archive>
void
load(Archive& ar, dfc::Upgrades::UpgradeParameters& o)
{
    time_t t;
    ar(make_nvp("time", t));
    o.mUpgradeTime = dfc::VirtualClock::from_time_t(t);
               ...
               AccountID const& accountID, Asset const& asset, int64_t delta)
{
    ...
    if (iter->second)
    {
        if (!dfc::addBalance(*iter->second, delta))
        {
            iter->second.reset();
        }
    }
                                        ...
                                        AbstractLedgerTxn& ltx)
{
    ...
    {
        auto trust = dfc::loadTrustLineWithoutRecord(ltx, accountID, asset);
...
prepareLiabilities(AbstractLedgerTxn& ltx, LedgerTxnHeader const& header)
{
    ...
    for (auto& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = dfc::loadAccount(ltx, accountOffers.first);
        if (!accountEntry)
        {
        ...
        for (auto& offerEntry : accountOffers.second)
        {
            ...
            {
                dfc::addNumEntries(header, accountEntry, -1);
            }
        ...
        for (auto const& assetLiabilities : liabilities)
        {
            ...
            {
                auto trustEntry =
                    dfc::loadTrustLine(ltx, accountOffers.first, asset);
                int64_t deltaSelling =
