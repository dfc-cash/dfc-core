...
void
HerderSCPDriver::restoreSCPState(uint64_t index, DfcBlockChainValue const& value)
{
...
HerderSCPDriver::checkCloseTime(uint64_t slotIndex, uint64_t lastCloseTime,
                                DfcBlockChainValue const& b) const
{
                                   ...
                                   std::set<Value> const& candidates)
{
    ...
    DfcBlockChainValue comp(h, 0, emptyUpgradeSteps, 0);
    ...
    std::vector<DfcBlockChainValue> candidateValues;

    for (auto const& c : candidates)
    {
        candidateValues.emplace_back();
        DfcBlockChainValue& sv = candidateValues.back();
...
HerderSCPDriver::valueExternalized(uint64_t slotIndex, Value const& value)
{
    ...
    catch (...)
    {
        ...
        // therefore contain a valid DfcBlockChainValue.
        CLOG(ERROR, "Herder") << "HerderSCPDriver::valueExternalized"
                              << " Externalized DfcBlockChainValue malformed";
...
void
HerderSCPDriver::nominate(uint64_t slotIndex, DfcBlockChainValue const& value,
                          TxSetFramePtr proposedSet,
                          DfcBlockChainValue const& previousValue)
{
