           ...
           std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    {
        auto accountB = dfc::loadAccountWithoutRecord(ltx, accountBID);
        if (!accountB)
        {
    ...
    { // entire offer is taken
        ...
        auto accountB = dfc::loadAccount(ltx, accountBID);
    ...
    if (numSheepSend != 0)
    {
        ...
        if (sheep.type() == ASSET_TYPE_NATIVE)
        {
            auto accountB = dfc::loadAccount(ltxInner, accountBID);
        ...
        {
            auto sheepLineAccountB =
                dfc::loadTrustLine(ltxInner, accountBID, sheep);
    ...
    if (numWheatReceived != 0)
    {
        ...
        {
            auto wheatLineAccountB =
                dfc::loadTrustLine(ltxInner, accountBID, wheat);
              ...
              bool isPathPayment, std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    if (!dfc::loadAccountWithoutRecord(ltx, accountBID))
    {
        throw std::runtime_error(
            "invalid database state: offer must have matching account");
    }
    ...
    {
        accountB = dfc::loadAccount(ltx, accountBID);
    }
                                   ...
                                   : CrossOfferResult::eOfferPartial;
    {
        ...
        {
            sellingWheatOffer.erase();
            accountB = dfc::loadAccount(ltxInner, accountBID);
