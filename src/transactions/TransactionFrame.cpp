                              ...
                              AccountID const& accountID)
{
    ...
    {
        // this is buggy caching that existed in old versions of the protocol
        auto res = dfc::loadAccount(ltx, accountID);
        if (res)
        {
    ...
    {
        return dfc::loadAccount(ltx, accountID);
    }
}
...
TransactionFrame::processFeeSeqNum(AbstractLedgerTxn& ltx)
{
    ...
    if (fee > 0)
    {
        ...
        dfc::addBalance(acc.balance, -fee);
...
{
    auto account = dfc::loadAccount(ltx, accountID);
    if (!account)
    {
                                      ...
                                      SignerKey const& signerKey) const
{
    ...
    {
        auto removed = dfc::addNumEntries(header, account, -1);
        assert(removed);
...
DfcBlockChainMessage
TransactionFrame::toDfcBlockChainMessage() const
{
