                            ...
                            LedgerTxnEntry const& offerEntry, bool isAcquire)
{
    ...
    auto loadAccountAndValidate = [&ltx, &sellerID]() {
        auto account = dfc::loadAccount(ltx, sellerID);
        if (!account)
        {
    ...
    auto loadTrustAndValidate = [&ltx, &sellerID](Asset const& asset) {
        auto trust = dfc::loadTrustLine(ltx, sellerID, asset);
        if (!trust)
        {
...
addBalance(LedgerTxnHeader const& header, LedgerTxnEntry& entry, int64_t delta)
{
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        if (!dfc::addBalance(newBalance, delta))
        {
            return false;
        }
    ...
    else if (entry.current().data.type() == TRUSTLINE)
    {
        ...
        if (!dfc::addBalance(newBalance, delta, tl.limit))
        {
            return false;
        }
        if (header.current().ledgerVersion >= 10)
                     ...
                     int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = dfc::addBalance(buyingLiab, delta, maxLiabilities);
        if (res)
        {
            if (acc.ext.v() == 0)
            {
                      ...
                      int64_t delta)
{
    ...
    if (entry.current().data.type() == ACCOUNT)
    {
        ...
        bool res = dfc::addBalance(sellingLiab, delta, maxLiabilities);
        if (res)
        {
            if (acc.ext.v() == 0)
            {
