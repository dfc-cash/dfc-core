...
ManageOfferOpFrame::checkOfferValid(AbstractLedgerTxn& ltxOuter)
{
    ...
    if (sheep.type() != ASSET_TYPE_NATIVE)
    {
        ...
        auto issuer = dfc::loadAccount(ltx, getIssuer(sheep));
        if (!issuer)
        {
    ...
    if (wheat.type() != ASSET_TYPE_NATIVE)
    {
        ...
        auto issuer = dfc::loadAccount(ltx, getIssuer(wheat));
        if (!issuer)
        {
...
ManageOfferOpFrame::doApply(Application& app, AbstractLedgerTxn& ltxOuter)
{
    ...
    { // modifying an old offer
        auto sellSheepOffer = dfc::loadOffer(ltx, getSourceID(), offerID);
        if (!sellSheepOffer)
        {
    ...
    if (mManageOffer.amount > 0)
    {
        ...
        if (header.current().ledgerVersion >= 10)
        {
            ...
            {
                auto sourceAccount =
                    dfc::loadAccountWithoutRecord(ltx, getSourceID());
