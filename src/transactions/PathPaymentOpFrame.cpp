...
PathPaymentOpFrame::doApply(Application& app, AbstractLedgerTxn& ltx)
{
    ...
    if (ltx.loadHeader().current().ledgerVersion < 8)
    {
        doesSourceAccountExist =
            (bool)dfc::loadAccountWithoutRecord(ltx, getSourceID());
    }

    if (!bypassIssuerCheck)
    {
        if (!dfc::loadAccountWithoutRecord(ltx, mPathPayment.destination))
        {
    ...
    {
        auto destination = dfc::loadAccount(ltx, mPathPayment.destination);
    ...
    else
    {
        ...
        {
            auto issuer =
                dfc::loadAccountWithoutRecord(ltx, getIssuer(curB));
            if (!issuer)
            {
        ...
        auto destLine =
            dfc::loadTrustLine(ltx, mPathPayment.destination, curB);
        if (!destLine)
        {
    ...
    for (int i = (int)fullPath.size() - 1; i >= 0; i--)
    {
        ...
        {
            if (!dfc::loadAccountWithoutRecord(ltx, getIssuer(curA)))
            {
                innerResult().code(PATH_PAYMENT_NO_ISSUER);
    ...
    if (curB.type() == ASSET_TYPE_NATIVE)
    {
        ...
        {
            sourceAccount = dfc::loadAccount(ltx, getSourceID());
            if (!sourceAccount)
            {
                innerResult().code(PATH_PAYMENT_MALFORMED);
                return false;
