...
TEST_CASE("payment", "[tx][payment]")
{
    ...
    SECTION("send XLM to an existing account")
    {
        for_all_versions(*app, [&] {
            ...
            {
                LedgerTxn ltx(app->getLedgerTxnRoot());
                auto rootAccount2 = dfc::loadAccount(ltx, root);
                auto rootAcc2 = rootAccount2.current().data.account();
                auto a1Account2 = dfc::loadAccount(ltx, a1);
    ...
    SECTION("rescue account (was below reserve)")
    {
        for_all_versions(*app, [&] {
            auto getMinBalance = [&] {
                LedgerTxn ltx(app->getLedgerTxnRoot());
                return dfc::getMinBalance(ltx.loadHeader(), 0);
            };
            ...
            for (auto const& data : withoutTrustLine)
            {
                SECTION(data.name)
                {
                    SECTION("without trustline")
                    {
                        ...
                        REQUIRE(!dfc::loadTrustLine(ltx, sendToSelf,
                                                        data.asset));
                    }
                }
            }
            ...
            for (auto const& data : withTrustLine)
            {
                SECTION(data.name)
                {
                    SECTION("with trustline and 0 balance")
                    {
                        ...
                        auto trust =
                            dfc::loadTrustLine(ltx, sendToSelf, data.asset);
                        REQUIRE(trust);
