                   ...
                   std::function<int(int)> getVote)
{
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        if (bal >= 0)
        {
            ...
            auto account = dfc::loadAccount(ltx, to.getPublicKey());
                  ...
                  std::function<int(int)> getVote, Application& app)
{
    ...
    for (auto w : winners)
    {
        ...
        if (ledgerVersion >= 10)
        {
            ...
            auto winner =
                dfc::loadAccount(ltx, getTestAccount(w).getPublicKey());
            toDoleToThis =
            ...
            std::function<int(int)> getVote, int expectedWinnerCount)
{
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        else
        {
            LedgerTxn ltx(app.getLedgerTxnRoot());
            auto account =
                dfc::loadAccount(ltx, getTestAccount(i).getPublicKey());
    ...
    for (int i = 0; i < nbAccounts; i++)
    {
        ...
        else
        {
            {
                LedgerTxn ltx(app.getLedgerTxnRoot());
                auto account = dfc::loadAccount(ltx, k.getPublicKey());
                auto const& ae = account.current().data.account();
